...
public class MonsterPlacerUtils {
	...
	public static String getButtonText(MobTag tag, ItemStack stack) {
		if (tag instanceof MobTagSlider) {
			...
			/*if(tag instanceof MobTagSliderByte){
				value = ((MobTagSliderByte)tag).getByte(stack);
			}else if(tag instanceof MobTagSliderShort){
				value = ((MobTagSliderShort)tag).getShort(stack);
			}else{
		...
		else return "No button text found for " + tag.getTranslatedName();
	}
	...
	public static MobTag[] getSpecificTagsForEntity(EntityLiving ent) {
		if (ent instanceof EntityCreeper) {
			...
			else return MobTag.ZOMBIEVILLAGER_SPECIFIC;
		}
