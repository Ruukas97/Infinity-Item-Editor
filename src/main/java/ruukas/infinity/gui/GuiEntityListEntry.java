...
public class GuiEntityListEntry implements GuiListExtended.IGuiListEntry
{
    private final Minecraft client;
    private final GuiEntityPicker entityPicker;
    private final GuiEntityList containingListSel;
    private final EntityLiving entity;
    private long lastClickTime;

    public GuiEntityListEntry(GuiEntityList listEntity, EntityLiving entity)
    {
        this.containingListSel = listEntity;
        this.entityPicker = listEntity.getGuiWorldSelection();
        ...
        if (this.client.gameSettings.touchscreen || isSelected)
        {
            ...
            if (this.worldSummary.markVersionInList())
            {
                ...
                if (this.worldSummary.askToOpenWorld())
                {
                    ...
                    if (j < 32)
                    {
                        this.entityPicker.setVersionTooltip(TextFormatting.RED + I18n.format("selectWorld.tooltip.fromNewerVersion1") + "\n" + TextFormatting.RED + I18n.format("selectWorld.tooltip.fromNewerVersion2"));
                    }
                }
                else
                {
                    ...
                    if (j < 32)
                    {
                        this.entityPicker.setVersionTooltip(TextFormatting.GOLD + I18n.format("selectWorld.tooltip.snapshot1") + "\n" + TextFormatting.GOLD + I18n.format("selectWorld.tooltip.snapshot2"));
                    }
                }
            }
            else
        ...
        if (this.worldSummary.askToOpenWorld())
        {
            this.client.displayGuiScreen(new GuiYesNo(new GuiYesNoCallback()
            {
                public void confirmClicked(boolean result, int id)
                {
                    if (result)
                    {
                        GuiEntityListEntry.this.loadWorld();
                    }
                    else
                    {
                        GuiEntityListEntry.this.client.displayGuiScreen(GuiEntityListEntry.this.entityPicker);
                    }
                }
    ...
    public void deleteWorld()
    {
        this.client.displayGuiScreen(new GuiYesNo(new GuiYesNoCallback()
        {
            public void confirmClicked(boolean result, int id)
            {
                if (result)
                {
                    GuiEntityListEntry.this.client.displayGuiScreen(new GuiScreenWorking());
                    ISaveFormat isaveformat = GuiEntityListEntry.this.client.getSaveLoader();
                    isaveformat.flushCache();
                    isaveformat.deleteWorldDirectory(GuiEntityListEntry.this.worldSummary.getFileName());
                    GuiEntityListEntry.this.containingListSel.refreshList();
                }
    ...
    {
        this.client.displayGuiScreen(new GuiWorldEdit(this.entityPicker, this.worldSummary.getFileName()));
    }

    public void recreateWorld()
    {
        this.client.displayGuiScreen(new GuiScreenWorking());
        GuiCreateWorld guicreateworld = new GuiCreateWorld(this.entityPicker);
    ...
    private void loadWorld()
    {
        ...
        {
            net.minecraftforge.fml.client.FMLClientHandler.instance().tryLoadExistingWorld(entityPicker, this.worldSummary);
        }
    }
