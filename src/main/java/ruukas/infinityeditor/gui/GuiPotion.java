...
public class GuiPotion extends GuiInfinity {
    ...
    private ArrayList<ResourceLocation> filteredPotions = new ArrayList<>(Potion.REGISTRY.getKeys());
    private ArrayList<ResourceLocation> potions = new ArrayList<>();
    ...
    public void initGui() {
        ...
        particleButton = addButton(new GuiInfinityButton(151, 15, height - 90, 80, 20, toggleButtonText));
        updateButtonText();
        resetPotions();
    ...
    private void updateButtonText() {
        toggleButtonText = I18n.format("gui.potion.showparticles." + (showParticles ? "1" : "0"));
        if (particleButton != null) particleButton.displayString = toggleButtonText;
    }
    ...
    protected void keyTyped(char typedChar, int keyCode) throws IOException {
        if (keyCode == 1) {
            ...
            filterTextField.textboxKeyTyped(typedChar, keyCode);
            if (filterTextField.isFocused()) filterPotions();
        }
    }
    ...
    private void filterPotions() {
        ...
        ArrayList<ResourceLocation> tempList = new ArrayList<>();
        for (ResourceLocation resourceLocation : potions) {
            PotionEffect potEff = new PotionEffect(Potion.REGISTRY.getObject(resourceLocation));
            System.out.println(I18n.format(potEff.getEffectName()).toLowerCase());
            if (!I18n.format(potEff.getEffectName()).toLowerCase().contains(filter)) continue;
            tempList.add(resourceLocation);
        }
        filteredPotions = tempList;
    }

    private void resetPotions() {
        potions.clear();
        for (ResourceLocation e : Potion.REGISTRY.getKeys()) {
            potions.add(e);
        }
    ...
    protected void mouseClicked(int mouseX, int mouseY, int mouseButton) throws IOException {
        ...
        if (Math.abs(mouseDist - r) < 16) {
            double angle = (2 * Math.PI) / filteredPotions.size();

            int lowDist = Integer.MAX_VALUE;
            Potion type = null;

            int i = 0;
            for (ResourceLocation key : filteredPotions) {
